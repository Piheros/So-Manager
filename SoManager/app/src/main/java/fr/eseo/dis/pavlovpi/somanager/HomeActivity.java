                ...
                JsonObjectRequest request = new JsonObjectRequest(Request.Method.GET, URLJPO, null,
                        new Response.Listener<JSONObject>() {
                            ...
                            public void onResponse(JSONObject response) {
                                // TODO Try to save response in file.txt
                                try {
                                    String data = response.toString();

                                    TextView total = (TextView )findViewById(R.id.textData2);
                                    total.setText(String.valueOf(response.toString()));

                                    FileOutputStream fileOutputStream = openFileOutput("dataResponseProject.txt", MODE_PRIVATE);
